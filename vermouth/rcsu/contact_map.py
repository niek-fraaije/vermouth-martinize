    ...
    for i1, i2 in product(np.arange(nresidues), np.arange(nresidues)):
        ...
        if (over > 0 or cont > 0):
            ...
            if over == 1 or (over == 0 and rcsu):
                ...
                contacts_list.append((int(G_a.nodes[a]['resid']), G_a.nodes[a]['chain'], mol_idx_a,
                                      int(G_b.nodes[b]['resid']), G_b.nodes[b]['chain'], mol_idx_b))
...
def _write_contacts(fout, all_contacts, ca_pos, G_list):
    ...
    for contact in all_contacts:
        ...
        msg = (f"R {int(count):6d} "
               ...
               f"{G_a.nodes[contact[2]]['chain']:1s} {int(G_a.nodes[contact[2]]['resid']):4d}    "
               f"{int(contact[1]):5d}  {G_b.nodes[contact[3]]['resname']:3s} "
               f"{G_b.nodes[contact[3]]['chain']:1s} {int(G_b.nodes[contact[3]]['resid']):4d}    "
